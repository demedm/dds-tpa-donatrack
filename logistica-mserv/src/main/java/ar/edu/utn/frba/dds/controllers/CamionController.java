...
public class CamionController {
  ...
  public Camion randomCamion() {
    return CamionRepositorio.Instance.buscarPorId((long) 1);
  }
  ...
  private Camion tryToFindCamion(String patente, Context ctx) {
    ...
    var camion = CamionRepositorio.Instance.buscarPorPatente(patente);
    if (camion == null) {
      ctx.status(404);
      return null;
    }
  ...
  public void saveCamion(Context ctx) {
    Camion camion = ctx.bodyValidator(Camion.class)
        ...
        .get();
    CamionRepositorio.Instance.registrar(camion);
